...
# ─── 3. hwarang-agent CLI PATH 노출 ─────────────────────────────────────
echo
echo "[3/7] hwarang-agent CLI 확인"
if command -v hwarang-agent &>/dev/null; then
    pass "hwarang-agent on PATH: $(command -v hwarang-agent)"
    # 버전 호출 (실패해도 fail 처리하진 않음 — CLI 가 --version/version 둘 다 지원할 수 있음)
    VERSION_OUT="$(hwarang-agent --version 2>/dev/null || hwarang-agent version 2>/dev/null || true)"
    if [[ -n "$VERSION_OUT" ]]; then
        pass "version 출력: $(echo "$VERSION_OUT" | head -1)"
    else
        warn "hwarang-agent 가 --version 응답 없음 (PATH 노출은 OK)"
    fi
else
    case "$OS" in
        darwin)
            warn "hwarang-agent PATH 미등록 — 다음 명령으로 등록:"
            echo "       sudo ln -sf '/Applications/Hwarang Grid Agent.app/Contents/Resources/binaries/hwarang-agent-$(uname -m | sed 's/arm64/aarch64/')-apple-darwin' /usr/local/bin/hwarang-agent"
            echo "       또는 앱 첫 실행 시 자동 등록 다이얼로그가 떴는지 확인"
            ;;
        linux)
            warn "hwarang-agent PATH 미등록 — .deb 재설치 또는 AppImage 의 경우:"
            echo "       installer/linux/appimage_install_cli.sh /path/to/Hwarang-Grid.AppImage"
            ;;
        msys*|mingw*|cygwin*)
            warn "hwarang-agent PATH 미등록 — 인스톨러를 재실행하거나 새 명령 프롬프트를 여세요"
            ;;
    esac
fi
