"""화랑 LoRA v11 학습 데이터 — 긴 응답 한국어 only 강제 6000건.

v9 (4000건) 의 약점:
  - 카테고리당 풀이 좁아 단조 반복 — 같은 형용사/명사/intro/outro 가
    수십 차례 중복돼, base 모델이 답변 다양성을 학습하지 못함.
  - v10 에서 태국어 (ที่) / 라틴확장 (á, ñ) 누출 잔존.
  - 4단어 영어 시퀀스 (예: "Out of Memory error") 가 통과.

v11 목표 (이것만 적용):
  1. 데이터 양 6000건 (카테고리별 1.5배).
  2. 슬롯 풀 (형용사/명사/intro/outro) 명시 + 매 샘플마다 random 결합.
  3. 시나리오 풀 카테고리별 1.5~2배 확장.
  4. 검증 강화: 태국어/라틴확장/베트남어/4단어 영어/환각/길이.

카테고리 (총 6000):
  L1 코드 리뷰 긴 분석 (1500)   응답 600자 이상
  L2 프로젝트 평가 (1200)        응답 400자 이상
  L3 기술 비교/장단점 (1200)     응답 500자 이상
  L4 디자인 시스템 분석 (1000)   응답 600자 이상
  L5 트러블슈팅 다단계 (1100)    응답 600자 이상
"""
...
_THAI = re.compile(r"[ก-๿]")                          # 신규
_LATIN_EXT = re.compile(r"[-ɏ]")           # 신규 — á, ñ 등
_VIET = re.compile(
    r"[ạáảãàặắằẳẵệếềểễỉịòóỏõộốồổỗợớờởỡụúủũựứừửữỳýỷỹ]"
)
_HALLUC = re.compile(
...
# 코드 블록 밖에서 4단어 이상 연속된 영어 (v9 5단어보다 엄격)
_LONG_EN_RUN = re.compile(r"(?:[A-Za-z][A-Za-z\-']*\s+){3,}[A-Za-z][A-Za-z\-']*")

MIN_LEN = {"L1": 600, "L2": 400, "L3": 500, "L4": 600, "L5": 600}


def is_clean_korean_v11(text: str, category: str) -> tuple[bool, str]:
    ...
    if _CJK.search(code_stripped):
        return False, "cjk"
    if _CYR.search(code_stripped):
        return False, "cyrillic"
    if _KANA.search(code_stripped):
        return False, "kana"
    if _THAI.search(code_stripped):
        return False, "thai"
    if _LATIN_EXT.search(code_stripped):
        return False, "latin_ext"
...
KOREAN_ADJ_POOL = [
    "인상적인", "주목할 만한", "두드러진", "탁월한", "효과적인",
    "효율적인", "안정적인", "유연한", "강력한", "체계적인",
    "직관적인", "명확한", "구체적인", "신중한", "균형 잡힌",
    "정교한", "세밀한", "포괄적인", "근본적인", "지속적인",
    "현실적인", "합리적인", "타당한", "적절한", "충분한",
    "필수적인", "선택적인", "보조적인", "결정적인", "중요한",
    "긍정적인", "부정적인", "중립적인", "객관적인", "주관적인",
    "단순한", "복잡한", "어려운", "쉬운", "흥미로운",
    "표준적인", "독특한", "전형적인", "예외적인", "보편적인",
    "최신의", "기본적인", "고급의", "기초적인", "심화된",
]  # 50종

KOREAN_NOUN_POOL = [
    "강점", "약점", "장점", "단점", "특성", "특징", "이점",
    "한계", "제약", "위험", "기회", "효과", "영향", "결과",
    "원인", "배경", "맥락", "기준", "지표", "성과",
    "변화", "차이", "관점", "방향", "전략", "전술", "계획",
    "구조", "체계", "흐름", "패턴", "구성", "조합", "관계",
    "의도", "목적", "목표", "단계", "절차", "방법",
    "도구", "수단", "기술", "기법", "원칙", "규칙",
    "조건", "환경", "여건", "상황",
]  # 50종
...
KOREAN_OUTRO_POOL = [
    "추가로 궁금하신 영역이 있으면 말씀해 주세요.",
    "필요하시면 특정 항목을 더 깊게 다뤄 드릴게요.",
    "도움이 되셨길 바라며, 더 구체적인 질문도 환영합니다.",
    "다음 단계로 무엇을 우선할지 함께 정리해 봐도 좋겠습니다.",
    "이 기준선을 토대로 추가 분석이 가능합니다.",
    "필요에 따라 더 세부 내용을 확장해 드릴게요.",
    "관련해서 더 살펴볼 항목이 있으면 알려주세요.",
    "추가 맥락이 있으면 더 정교한 답변이 가능합니다.",
    "실행에 들어가실 때 막히는 부분이 있으면 다시 알려주세요.",
    "여기까지가 큰 흐름이고, 세부는 상황 따라 조정 가능합니다.",
    "더 들여다볼 영역이 있으면 알려주세요.",
    "다음 결정에 활용하시기 좋게 정리했습니다.",
    "추가 자료가 있다면 더 정확한 진단이 가능합니다.",
    "여기서 출발해서 단계적으로 다듬어 가시면 됩니다.",
    "본문에서 다루지 못한 부분이 있으면 보충해 드릴게요.",
]  # 15종


def pick_adj() -> str:
    return random.choice(KOREAN_ADJ_POOL)


def pick_noun() -> str:
    return random.choice(KOREAN_NOUN_POOL)


def pick_intro() -> str:
    return random.choice(KOREAN_INTRO_POOL)


def pick_outro() -> str:
    return random.choice(KOREAN_OUTRO_POOL)


def sample_distinct(pool: list[str], k: int) -> list[str]:
    """풀에서 중복 없이 k 개 샘플. 풀이 부족하면 가능한 만큼."""
    k = min(k, len(pool))
    return random.sample(pool, k=k)


# ============================================================
# 카테고리별 intro/outro (v9 의 풀 그대로 + 확장)
# ============================================================
...
L_INTRO = {
    "L1": [
        ...
        "전체적인 구조는 이해하기 쉽게 짜여 있고, 큰 결함은 보이지 않았습니다.",
        "리뷰 포인트를 구조, 가독성, 안정성, 성능 네 갈래로 나눠 정리해 봤습니다.",
        "처음 보는 사람이 흐름을 따라가기 좋은 코드입니다.",
        "코드 동작 자체는 의도대로 흐르고, 다만 디테일에 다듬을 여지가 보입니다.",
        "리뷰 관점에서 핵심 짚을 부분을 다섯 갈래로 정리했습니다.",
        "구조와 의도를 따라가며 점검 포인트를 추려봤습니다.",
    ],
    "L2": [
        "지금 보여 주신 정보 안에서 가능한 평가를 정리해 보겠습니다.",
        "초기 단계 프로젝트라는 점을 고려하면 전반적으로 합리적인 출발선에 있다고 봅니다.",
        "현재 구성과 목표를 함께 놓고 보면 강점과 약점이 비교적 또렷하게 나뉩니다.",
        "방향성 자체는 시장에 충분히 통할 만하다고 봅니다.",
        "결론부터 말씀드리면 핵심 가설을 빨리 검증할 수 있느냐가 가장 큰 변수입니다.",
        "주신 컨텍스트 기준으로 강점/약점/우선순위를 짚어 드리겠습니다.",
        "현 상태에서 살펴봐야 할 리스크 위주로 정리해 봤습니다.",
        "단계별로 무엇부터 검증해야 할지 우선순위를 잡아 봤습니다.",
    ],
    "L3": [
        ...
        "두 도구의 차이는 결국 팀의 운영 스타일과 잘 맞는 쪽이 어느 쪽인가로 귀결됩니다.",
        "현장 도입 사례 기준으로 양쪽의 흐름을 비교해 드리겠습니다.",
        "선택 기준을 두 축(운영 부담, 생태계 성숙도)으로 나눠 정리합니다.",
    ],
    "L4": [
        ...
        "디자인 일관성 측면에서 토큰화 수준부터 점검해 보겠습니다.",
        "접근성과 다크 모드 두 축에 초점을 맞춰 정리했습니다.",
        "토큰/컴포넌트/패턴 세 층으로 나눠 진단하겠습니다.",
    ],
    "L5": [
        ...
        "환경 변수, 의존성 락, 비동기 흐름 순으로 살펴보겠습니다.",
        "재현 조건을 좁히는 것부터 시작해서 가설을 검증하는 순서로 풀어봅니다.",
        "메시지 표면이 아니라 한두 호출 위에서 원인을 찾는 게 빠릅니다.",
    ],
}
...
L_OUTRO = {
    "L1": [
        ...
        "구조적으로 큰 결함이 없으니, 작은 단위로 다듬으시면 충분합니다.",
        "검토하신 뒤 우선순위만 정해 나누면 매끄럽게 진행될 겁니다.",
        "위 항목들은 개별 PR 단위로 나누는 것이 안전합니다.",
    ],
    "L2": [
        ...
        "지표 한두 개에 집중해 실행하면 다음 분기 평가가 한결 또렷해집니다.",
        "리스크는 작게 자주 검증하는 흐름이 가장 안전합니다.",
        "위 우선순위 기준으로 분기 계획을 다듬어 보세요.",
    ],
    "L3": [
        ...
        "작은 PoC 로 1~2 주 정도 다뤄 보고 결정하시는 흐름을 권장합니다.",
        "운영 부담을 미리 측정하면 도입 후 후회 비용이 크게 줄어듭니다.",
        "최종 결정은 팀이 가장 익숙한 쪽에 살짝 가중을 두시는 게 안전합니다.",
    ],
    "L4": [
        ...
        "이름 합의부터 시작해서 토큰을 단계적으로 정리하시면 됩니다.",
        "접근성 체크리스트를 한 번 잡아 두면 신규 컴포넌트가 가벼워집니다.",
        "토큰화 → 변형 축소 → 패턴 문서화 순서를 추천합니다.",
    ],
    "L5": [
        ...
        "재현 조건이 잡히면 절반은 해결된 거나 다름없습니다.",
        "임시 우회와 근본 수정은 분리해 진행하시면 안전합니다.",
        "원인 좁히기 → 우회 → 근본 수정 → 회귀 테스트 순으로 진행하세요.",
    ],
}
...
def render_adj_noun_line(template: str) -> str:
    """`{adj}` `{noun}` 자리 표시자를 풀에서 뽑아 채워 넣음."""
    adjs_used: set[str] = set()
    nouns_used: set[str] = set()

    def replace_adj(_):
        for _attempt in range(8):
            cand = pick_adj()
            if cand not in adjs_used:
                adjs_used.add(cand)
                return cand
        return pick_adj()
    ...
    def replace_noun(_):
        for _attempt in range(8):
            cand = pick_noun()
            if cand not in nouns_used:
                nouns_used.add(cand)
                return cand
        return pick_noun()

    out = re.sub(r"\{adj\}", replace_adj, template)
    out = re.sub(r"\{noun\}", replace_noun, out)
    return out


# ============================================================
# L1 코드 리뷰 (1500) — v9 풀 + 확장
# ============================================================
...
```"""),
    ("Python", "배치 잡 스케줄러", """```python
def run_daily_batch():
    for tenant in tenants:
        report = build_report(tenant)
        send_email(tenant.admin_email, report)
```"""),
    ("Python", "스트림 처리", """```python
...
```"""),
    ("Swift", "Codable 디코딩", """```swift
let user = try JSONDecoder().decode(User.self, from: data)
```"""),
    ("Kotlin", "코루틴 스코프", """```kotlin
viewModelScope.launch {
    val user = repo.fetchUser(id)
    _state.value = State.Loaded(user)
}
```"""),
    ("Kotlin", "data class", """```kotlin
data class User(val id: Long, val name: String, val email: String)
```"""),
    ("Ruby", "Rails 컨트롤러", """```ruby
def create
...
L1_REVIEW_POINTS = {
    "FastAPI 라우터": [
        ...
        "트랜잭션 경계가 commit 한 줄에만 묶여 있어, 예외가 나면 세션 정리가 모호합니다.",
    ...
    "동기 파일 처리 루프": [
        ...
        "I/O 가 동기로 직렬화돼 있어 멀티프로세싱이나 asyncio 로 병렬화하면 처리 시간이 줄어듭니다.",
        "에러가 나는 파일이 있어도 전체가 멈추지 않도록, 개별 파일 단위 예외 처리를 보강할 필요가 있습니다.",
    ],
    "재귀 피보나치": [
        "같은 부분 문제를 반복 호출해 호출 수가 지수적으로 폭증합니다. functools.lru_cache 한 줄이면 줄어듭니다.",
        "재귀 깊이가 n 에 비례해 늘어나 큰 n 에서는 RecursionError 위험이 있습니다.",
        "반복문이나 iterative DP 로 바꾸면 시간/공간 복잡도가 크게 개선됩니다.",
        "타입 힌트와 docstring 이 있으면 사용처에서 음수나 매우 큰 값의 동작을 짐작하기 쉬워집니다.",
    ],
    "SQL 인젝션 위험 코드": [
    ...
    "race condition": [
        "공유 변수에 동시 접근하는 고루틴이 락 없이 작업해 결과가 예측 불가합니다.",
        "sync.Mutex 또는 atomic 패키지로 보호하면 race detector 가 깨끗해집니다.",
        "go test -race 로 자동 탐지를 활성화해 두는 것이 안전합니다.",
        "공유 상태를 채널로 전달하는 패턴으로 바꾸면 동시성 의도가 더 또렷해집니다.",
    ],
}
...
L1_GENERIC_POINTS = [
    ...
    "로깅 메시지에 컨텍스트를 함께 남기면 운영 추적이 훨씬 수월합니다.",
    ...
    "외부 의존성 호출에는 타임아웃과 재시도 정책을 명시적으로 두는 것이 안전합니다.",
    "주석이 코드 동작과 어긋나 있는지 가장 흔한 함정입니다. 코드를 바꿨다면 주석도 함께 점검해 주세요.",
    "함수 길이가 일정 줄 수 이상으로 늘어나면, 의도 단위 함수로 쪼개는 것이 후속 변경에 유리합니다.",
    "에러 메시지는 사용자가 다음 행동을 결정할 수 있을 만큼 구체적인지 다시 한번 살펴보세요.",
    "외부 의존성을 직접 부르기보다 인터페이스로 분리해 두면 테스트 더블 적용이 쉬워집니다.",
    "스레드 안전성을 가정하는 변수에는 명시적인 동기화 수단을 함께 두는 것이 안전합니다.",
    "동일 로직이 두세 군데에서 반복되면, 중복 제거를 통해 회귀 비용을 줄일 수 있습니다.",
]
...
def gen_l1():
    ...
    while len(points) < n_points and generic_pool:
        cand = generic_pool.pop()
        if cand not in points:
            points.append(cand)

    # 슬롯 풀로 본문 한 줄 다양화
    flavor = render_adj_noun_line(
        f"전체 인상은 {{adj}} 흐름이며, 코드의 {{noun}} 와 {{noun}} 가 또렷하게 드러납니다."
    )
...
L2_PROJECTS = [
    ...
    "스마트홈 허브, 디바이스 호환성 30종, 펌웨어 업데이트 수동",
    ...
    "법률 문서 자동화 SaaS, 변호사 30명 사용, 환각 우려",
    "보안 침해 대응 플랫폼, MSP 2곳 도입, 오탐률 높음",
    "어린이 교육 콘텐츠 구독, 부모 사용자 5천, 이탈률 30 퍼센트",
    "헬스 트래킹 웨어러블 동반 앱, 활성 5만, 데이터 정확도 이슈",
    "기업 문서 검색 RAG, 사내 5팀 도입, 응답 품질 편차",
    "스타트업 채용 보드, 무료 등록, 기업 측 후기 부족",
    "푸드테크 다크 키친 운영 도구, 매장 8곳, 주문 폭주 시 안정성 약점",
    "B2B 인보이스 자동화, 회계팀 도입, OCR 정확도 90 퍼센트",
    "공공 데이터 시각화 포털, 월 방문 2만, 갱신 주기 수동",
    "메타버스 협업 공간, 동시 접속 100명, 성능 병목",
    "AI 음성 비서 모바일, 사용자 1만, 한국어 인식률 편차",
    "IoT 펌웨어 OTA 플랫폼, 디바이스 5종, 롤백 정책 미흡",
    "유아용 AI 동화 앱, 부모 후기 긍정, 콘텐츠 추가 비용 부담",
]
...
L2_STRENGTHS = [
    ...
    "도메인 전문성을 가진 인력이 있다면, 이는 다른 팀이 쉽게 따라가기 힘든 강점입니다.",
    "MVP 가 빨리 시장에 나가 있는 점은 정성·정량 피드백을 동시에 모을 수 있어 큰 자산입니다.",
    "API 우선 설계 덕분에 클라이언트가 늘어도 백엔드 부담이 선형적으로만 늘어납니다.",
    "초기 사용자 풀이 좁지만 충성도가 높다면, 정성 인터뷰의 신호 대 잡음비가 매우 좋아집니다.",
    "팀 규모 대비 출시 속도가 빠른 편이라, 학습 사이클 자체가 강점이 됩니다.",
    "내부 운영 자동화 비중이 높아, 단위 운영 비용이 사용자 증가만큼 비례해 늘지 않습니다.",
]

L2_WEAKNESSES = [
    "수익 모델이 단일하거나 검증되지 않은 상태라 자금 흐름이 외부 변수에 크게 흔들릴 수 있습니다.",
    "사용자 이탈 원인을 정량 지표가 아니라 직관으로 추정하는 단계라, 의사 결정이 느려질 위험이 있습니다.",
    ...
    "지표 정의가 팀 내부에서 일관되지 않으면, 의사 결정마다 같은 숫자를 두고도 결론이 갈릴 수 있습니다.",
    "마케팅 채널이 단일하면 그 채널 비용이 오를 때 곧바로 전체 성장률이 둔화됩니다.",
    "온보딩 단계 이탈이 잡히지 않으면, 그 위 단계 지표의 신뢰도가 크게 떨어집니다.",
]
...
L2_NEXT = [
    ...
    "온보딩 첫 5분 안에 핵심 가치를 체험시킬 수 있도록, 그 흐름을 최우선 과제로 잡아 보세요.",
    "이탈 사용자 인터뷰는 적극 사용자 인터뷰만큼 가치가 있습니다. 정기적인 채널을 만들어 두세요.",
    "각 채널별 단위 획득 비용을 측정해, 가장 효율이 좋은 한두 채널에 자원을 몰아 주세요.",
]
...
def gen_l2():
    ...
    flavor = render_adj_noun_line(
        "현재 프로젝트의 {noun} 는 {adj} 흐름을 보이고, 동시에 {noun} 측면에서 점검이 필요합니다."
    )
...
L3_PAIRS = [
    ...
    ("Auth0", "Clerk", "인증 SaaS"),
    ("Stripe", "Toss", "결제"),
    ("Datadog", "Grafana", "관찰 가능성"),
    ("Sentry", "Bugsnag", "에러 트래킹"),
    ("Hugging Face Transformers", "vLLM", "LLM 추론 스택"),
    ("Bazel", "Nx", "모노레포 빌드"),
    ("Turborepo", "Nx", "모노레포 도구"),
    ("Tauri", "Electron", "데스크탑 앱 프레임워크"),
    ("Flutter", "React Native", "크로스플랫폼 모바일"),
    ("SwiftUI", "UIKit", "iOS UI"),
    ("Jetpack Compose", "XML 레이아웃", "Android UI"),
    ("Polars", "Pandas", "데이터 분석"),
    ("DuckDB", "SQLite", "분석용 임베디드 DB"),
    ("Snowflake", "BigQuery", "데이터 웨어하우스"),
    ("Airflow", "Dagster", "데이터 파이프라인"),
    ("dbt", "SQLMesh", "SQL 변환 도구"),
]


def gen_l3():
    a_name, b_name, kind = random.choice(L3_PAIRS)
    q_templates = [
        f"{a_name} vs {b_name} 비교 해 줘.",
        f"{a_name} 와 {b_name} 중에 뭐가 더 나아? ({kind})",
        f"{kind} 고르고 있는데 {a_name} 랑 {b_name} 장단점 알려줘.",
        f"{a_name} 에서 {b_name} 로 옮기려는데 가치가 있을까?",
        f"{a_name} 와 {b_name} 의 차이를 표로 보여줘.",
        f"{kind} 결정 회의 전에 {a_name}/{b_name} 정리 부탁드립니다.",
    ]
    q = random.choice(q_templates)
    ...
    a_strengths = [
        f"{a_name} 는 학습 곡선이 비교적 완만하고 생태계가 넓다는 점이 큰 강점입니다.",
        f"{a_name} 는 커뮤니티 답변과 레퍼런스가 풍부해 막혔을 때 해결 비용이 낮습니다.",
        f"{a_name} 는 도구 체인이 안정적이고, 운영 경험을 가진 인력 풀이 두텁습니다.",
        f"{a_name} 는 점진적 도입이 쉬워, 기존 코드와 섞어 쓰는 부담이 작습니다.",
        f"{a_name} 는 장기 호환성이 검증된 영역이 많아, 마이그레이션 비용을 예측하기 쉬운 편입니다.",
    ]
    b_strengths = [
        f"{b_name} 는 최신 설계 철학을 더 적극적으로 반영해 성능과 개발 경험에서 인상적인 장점이 있습니다.",
        f"{b_name} 는 보일러플레이트가 적어 작은 팀이 빠르게 결과를 내는 데 유리합니다.",
        f"{b_name} 는 핵심 영역에서 더 일관된 API 를 제공해 학습 후 생산성이 빠르게 올라옵니다.",
        f"{b_name} 는 차세대 기능을 빠르게 흡수해, 미래 호환성에서 비교적 유리한 위치를 점하고 있습니다.",
        f"{b_name} 는 디폴트 동작이 안전하게 잡혀 있어, 초기 구성 비용이 더 낮습니다.",
    ]
    a_weaknesses = [
        f"{a_name} 는 오래된 패턴이 누적돼 있어, 새 코드와 옛 코드의 스타일 차이가 학습 부담으로 돌아올 수 있습니다.",
        f"{a_name} 는 일부 영역에서 디폴트 동작이 보수적이라, 적극적인 최적화에는 추가 설정이 필요합니다.",
        f"{a_name} 는 강력한 만큼 자유도가 커서, 팀 컨벤션이 약하면 일관성이 쉽게 무너집니다.",
        f"{a_name} 는 일부 기능이 코어 외부 플러그인에 의존해, 의존성 관리 부담이 늘어날 수 있습니다.",
    ]
    b_weaknesses = [
        f"{b_name} 는 비교적 새로워 운영 사례 풀이 좁고, 장기 호환성과 인력 수급 측면에서 변동성이 더 큽니다.",
        f"{b_name} 는 일부 엣지 케이스에서 도구 지원이 아직 부족할 수 있어, 도입 전에 PoC 가 거의 필수입니다.",
        f"{b_name} 는 생태계가 빠르게 변해 매년 베스트 프랙티스가 달라지는 부담이 있습니다.",
        f"{b_name} 는 한국어 자료가 상대적으로 적어, 초기 학습 비용이 영문 자료 의존도가 큽니다.",
    ]
    ...
    flavor = render_adj_noun_line(
        f"두 도구의 {{noun}} 는 {{adj}} 측면에서 갈리며, 각자의 {{noun}} 가 또렷합니다."
    )
    ...
    parts.append("## 선택 가이드")
    parts.append(
        f"- 안정적인 운영 사례와 기존 자산이 중요하다면 {a_name} 가 무난한 출발선입니다.\n"
        f"- 새로운 설계 철학과 작은 팀의 속도를 중시한다면 {b_name} 가 매력적인 선택이 됩니다.\n"
...
L4_POINTS = [
    ...
    "타이포 스케일은 모듈러 비율을 기준으로 잡되, 실제 화면에서 충분히 구분되는지 눈으로 다시 확인해야 합니다.",
    "포커스 링은 키보드 사용자의 핵심 단서이므로, 디자인적으로 거슬린다는 이유로 제거해서는 안 됩니다.",
    "텍스트와 배경의 대비비 4.5:1 이상은 본문 텍스트의 최소선으로 잡는 것이 안전합니다.",
    "다크 모드에서는 단순 색상 반전이 아니라 표면 위계(elevation) 를 더 부드럽게 재정의해야 합니다.",
    "버튼 사이즈는 4 단계 이상으로 늘리지 말고 sm/md/lg 세 단계로 묶는 것을 권합니다.",
    "상태 색상은 색상에만 의존하지 말고 아이콘과 텍스트로 같은 의미를 한 번 더 전달해야 합니다.",
    ...
    "스크린리더 사용자를 위해 핵심 액션 버튼에는 시각 텍스트 외에도 명시적인 라벨을 두는 것이 안전합니다.",
    "모바일 터치 타깃은 최소 44 픽셀 정도를 확보해야 손가락 오조작을 줄일 수 있습니다.",
    "데이터 시각화에서는 색약 사용자를 위해 색상만이 아니라 모양/패턴 차이를 함께 활용하는 것이 좋습니다.",
    "사이드 네비게이션은 접힘 상태에서도 아이콘만으로 의미가 전달되는지 한 번 더 점검해야 합니다.",
    "차트 범례는 데이터 영역 근처에 두고, 시선 이동 거리를 짧게 잡는 것이 가독성에 도움이 됩니다.",
    "폼 라벨은 가능한 한 입력 필드 위에 두는 것이 모바일 환경 가독성을 더 안정적으로 잡아 줍니다.",
]
...
def gen_l4():
    ...
    table_rows = [
        ...
        ("접근성", "키보드 포커스, 대비비, 명시적 라벨", "스크린리더 시나리오로 점검"),
    ...
    flavor = render_adj_noun_line(
        "디자인 측면에서 {adj} 일관성이 보이고, 토큰의 {noun} 와 컴포넌트의 {noun} 가 또렷이 드러납니다."
    )
...
```"""),
    ("Python", "메모리 누수 의심", """```
프로세스 RSS 가 매시간 50MB 씩 증가, GC 효과 없음
```"""),
...
MaxListenersExceededWarning: 11 listeners added.
```"""),
...
Hydration failed because the initial UI does not match server.
```"""),
...
```"""),
    ("Redis", "MISCONF Redis save", """```
MISCONF Redis is configured to save snapshots but cannot persist.
```"""),
...
build terminated: out-of-memory event detected
```"""),
    ("Browser", "CORS preflight 차단", """```
blocked by CORS policy
```"""),
    ("Browser", "Mixed content", """```
Mixed Content: page loaded over HTTPS requested insecure http resource
```"""),
...
operation failed because required record not found
```"""),
    ("Mac", "코드사이닝 실패", """```
...
adb: failed to install apk: INSUFFICIENT_STORAGE
```"""),
    ("iOS", "code signing required", """```
error: Signing requires a development team.
```"""),
    ("일반", "race condition", """```
멀티 워커 환경에서 카운터 값이 간헐적으로 누락됨
```"""),
    ("일반", "production 전용 timeout", """```
로컬은 멀쩡한데 운영에서만 30 초 후 504 응답
```"""),
    ("일반", "메모리 누수", """```
하루마다 메모리 200MB 증가, 재시작 후 다시 누적
```"""),
    ("일반", "캐시 정합성 깨짐", """```
DB 는 최신 값인데 사용자에게는 옛 값이 노출됨
```"""),
]
...
L5_HYPOTHESES = [
    ...
    "공유 자원에 동시 접근이 일어나면서 일정 부하 이상에서만 드러나는 경합이 있습니다.",
    ...
    "메모리 누수 의심 시, 장기 보관되는 컬렉션에 항목이 계속 쌓이는 경우를 먼저 살펴보는 것이 좋습니다.",
    "DNS 또는 네트워크 정책이 지역별로 다르게 적용돼, 특정 리전에서만 실패하는 사례도 있습니다.",
]
...
L5_VERIFY = [
    ...
    "프로세스 메모리 스냅샷을 일정 간격으로 떠서, 어떤 객체가 계속 누적되는지 비교해 보세요.",
    "외부 API 응답 스키마가 변경됐는지 의심된다면, 응답 본문 자체를 그대로 로그에 한 번 떨어뜨려 비교해 보세요.",
]

L5_PREVENT = [
    "회귀 방지용 단위 테스트를 한 개라도 좋으니 반드시 추가해, 같은 시나리오가 자동으로 잡히도록 만드세요.",
    "원인 분석 결과를 ADR 또는 사건 노트로 짧게라도 남기면 다음 회귀에서 진입 비용이 크게 줄어듭니다.",
    "운영 환경의 핵심 지표에 임계 알림을 걸어 두면 같은 류의 사고를 조기에 잡을 수 있습니다.",
    "외부 API 의존부에는 응답 스키마 검증 한 단을 두어, 변경을 즉시 감지할 수 있도록 하세요.",
    "동시성이 의심되는 영역에는 의도적으로 race 를 유도하는 부하 시험을 주기적으로 돌려 보면 좋습니다.",
    "사건 후 재발 방지 체크리스트를 코드 리뷰 단계에도 짧게 끼워 넣으면 효과가 큽니다.",
    "임시 우회 패치는 추적 이슈와 함께 남기고, 일정 기한 후 자동으로 점검 알림이 오도록 잡아 두세요.",
]
...
def gen_l5():
    ...
    flavor = render_adj_noun_line(
        "에러의 {noun} 는 표면이 아니라 한 단 위의 {noun} 에서 비롯됐을 가능성이 큽니다."
    )
...
CATEGORIES: list[tuple[str, int, Callable[[], tuple[str, str]]]] = [
    ("L1_code_review", 1500, gen_l1),
    ("L2_project_eval", 1200, gen_l2),
    ("L3_tech_compare", 1200, gen_l3),
    ("L4_design_system", 1000, gen_l4),
    ("L5_troubleshoot", 1100, gen_l5),
]
...
def build_sample(gen: Callable[[], tuple[str, str]], category: str, max_retries: int = 3):
    ...
    for _ in range(max_retries):
        q, a = gen()
        ok, reason = is_clean_korean_v11(a, category[:2])
        if ok:
            return {
                "messages": [
                    {"role": "user", "content": q},
...
def validate_file(path: str) -> int:
    """jsonl 파일을 다시 읽어 모든 assistant 응답을 재검증."""
    dirty = 0
    total = 0
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            for m in obj.get("messages", []):
                ...
                ok, reason = is_clean_korean_v11(content, cat)
                if not ok:
                    dirty += 1
...
def main():
    parser = argparse.ArgumentParser(description="화랑 v11 긴 응답 한국어 only SFT 데이터 빌더")
    parser.add_argument("--output", default="../../data/sft/ko_prose_v11.jsonl")
    ...
    print(f"[build_ko_prose_v11] {len(samples)} samples → {out}")
