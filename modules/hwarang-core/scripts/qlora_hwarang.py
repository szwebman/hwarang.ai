"""화랑 AI QLoRA 파인튜닝 스크립트.

RTX 5090 32GB 1장에서 실행 가능.
bitsandbytes INT4 양자화 + LoRA 어댑터(~30MB) = ~20GB VRAM.

사용법:
    poetry run python scripts/qlora_hwarang.py \
        --model-path /mnt/nvme2/hwarang/models/qwen2.5-32b \
        --data data/sft/hwarang_all_v2.jsonl \
        --output /mnt/nvme2/hwarang/models/hwarang-all-v2 \
        --epochs 10

필요 패키지:
    poetry run pip install transformers accelerate peft trl bitsandbytes
"""
