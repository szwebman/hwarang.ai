...
const TRIGGERS: { key: TriggerKey; label: string; path: string; icon: string; description: string }[] = [
  {
    key: "verify",
    label: "지금 자가 검증 실행",
    path: "/api/admin/hlkm/admin/verify/run",
    icon: "🔍",
  ...
  },
  {
    key: "hrag",
    label: "HRAG 법률 동기화",
    path: "/api/admin/hlkm/admin/hrag/sync",
    icon: "⚖️",
  ...
  },
  {
    key: "halflife",
    label: "반감기 모델 재학습",
    path: "/api/admin/hlkm/admin/halflife/retrain",
    icon: "⏳",
...
export default function TriggerPanel() {
  ...
  const run = async (t: (typeof TRIGGERS)[number]) => {
    ...
    setRunning(t.key);
    try {
      const resp = await fetch(t.path, { method: "POST" });
