...
export default function GapsPage() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const resp = await fetch("/api/admin/hlkm/stats/gaps");
      if (resp.ok) {
  ...
  const doAction = async (gap: Gap, action: "search" | "ignore") => {
    ...
    try {
      const path =
        action === "search"
          ? `/api/admin/hlkm/admin/gaps/${gap.id}/search`
          : `/api/admin/hlkm/admin/gaps/${gap.id}/ignore`;
  ...
  const manualAdd = async (gap: Gap) => {
    ...
    try {
      const resp = await fetch(`/api/admin/hlkm/admin/gaps/${gap.id}/fill`, {
        method: "POST",
