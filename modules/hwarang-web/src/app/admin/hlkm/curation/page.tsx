...
export default function CurationPage() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const resp = await fetch("/api/admin/hlkm/facts/pending");
      if (resp.ok) {
  ...
  const act = async (id: string, action: "approve" | "reject", body?: any) => {
    ...
    setMessage(null);
    try {
      const resp = await fetch(`/api/admin/hlkm/facts/${id}/${action}`, {
        method: "POST",
  ...
  const bulkApproveHighQuality = async () => {
    ...
    for (const f of targets) {
      try {
        const resp = await fetch(`/api/admin/hlkm/facts/${f.id}/approve`, { method: "POST" });
        if (resp.ok) okCount++;
      } catch {}
    }
    setBusy(null);
