/**
 * Trusted Sources 관리자 프록시 (목록 / 생성)
 *
 * Next.js (admin) → FastAPI (hwarang-api)
 *   GET   /api/trusted-sources       → ${HWARANG_API_URL}/api/sources
 *   POST  /api/trusted-sources       → ${HWARANG_API_URL}/api/sources
 *
 * 인증: Authorization: Bearer <admin_token> 검증 → ADMIN/SUPER_ADMIN 만 통과
...
export async function GET(request: NextRequest) {
  ...
  const target = `${HWARANG_API_URL}/api/sources${search || ""}`;
...
export async function POST(request: NextRequest) {
  ...
  return forward(`${HWARANG_API_URL}/api/sources`, {
    method: "POST",
    headers: backendHeaders(guard.userId, guard.role),
    body: body || undefined,
  });
