 ...
 * CodePattern 목록 프록시.
 *
 *   GET /api/research/code/patterns?language=&category=&framework=&limit=
 *     → ${HWARANG_API_URL}/api/research/patterns
 *       (FastAPI 측은 /api/research/patterns 가 코드 패턴 — 기존 경로 재사용)
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    const resp = await fetch(`${HWARANG_API_URL}/api/research/patterns${search || ""}`, {
      method: "GET",
      headers: {
