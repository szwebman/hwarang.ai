/**
 * Research Tech Trends 프록시 (Group C — 코드 + 디자인 통합).
 *
 *   GET /api/research/tech-trends?domain=&weeks=&only_emerging=
 *     → ${HWARANG_API_URL}/api/research/tech-trends
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    const resp = await fetch(`${HWARANG_API_URL}/api/research/tech-trends${search || ""}`, {
      method: "GET",
      headers: {
